...
for (i in 1:length(amenities)) {
  a = str_split(amenities[i], ",")
  a = unlist(a)
  b = str_trim(a)
  b = str_replace_all(b, " .", "")
  ame = unique(c(ame,b))
}
amenities_cleaned = data.frame(t(ame),stringsAsFactors = F)
colnames(amenities_cleaned) = ame
amenities_cleaned = amenities_cleaned[-1,]
for (i in 35119:length(amenities)) {
  a = str_split(amenities[i], ",")
  a = unlist(a)
  b = str_trim(a)
  b = str_replace_all(b, " .", "")
  for (j in 1:length(b)) {
    amenities_cleaned[i,b[j]] = T
  }
}
amenities_cleaned = amenities_cleaned[,c("Hotater","Iron","Laptopriendlyorkspace","TV","Shampoo","Carbononoxideetector","Hairryer","Hangers","Aironditioning","Smokeetector","Essentials","Kitchen","Heating","Wifi")]
for (i in 1:14) {
  amenities_cleaned[is.na(amenities_cleaned[,i]),i] = F
}
data = cbind(data,amenities_cleaned)
#
...
#run forest3
set.seed(5739)
forest3 = randomForest(price~., data = data, ntree = 1000)
# Predict
pred = predict(forest3)
# Calculate RMSE
...
#run forest4
set.seed(739)
forest4 = randomForest(price~., data = data, ntree = 1200)
# Predict
pred = predict(forest4)
# Calculate RMSE
rmse_analysis = sqrt(mean((pred-data$price)^2))
rmse_analysis
#58.2797
...
#run bag1
set.seed(395)
bag1 = randomForest(price~., data = data, ntree = 1000, mtry = ncol(data)-1)
# Predict
pred = predict(bag1)
# Calculate RMSE
rmse_analysis = sqrt(mean((pred-data$price)^2))
...
for (j in 1:15) {
  for (i in 1:59) {
    set.seed(936)
    split = createDataPartition(y = data$price, p = 0.01*j, list = F)
    trydata = data[split,]
    set.seed(1213)
    forest3 = randomForest(price~., data = trydata, ntree = 10*j, mtry = i)
    # Predict
    pred = predict(forest3)
    # Calculate RMSE
    rmse_analysis = sqrt(mean((pred-trydata$price)^2))
    rmse[(10*j+i),1] = rmse_analysis
    rmse[(10*j+i),2] = j
    rmse[(10*j+i),3] = i
  }
}
colnames(rmse) = c("rmse","samplesize","ntree")
ggplot(aes(x = ntree, y = rmse, color = as.factor(samplesize)), data = rmse) +
  geom_point()
...
for (j in 3:15) {
  for (i in seq(1,20,2)) {
    ...
    trydata = data[split,]
    set.seed(1213)
    forest3 = randomForest(price~., data = trydata, ntree = 10*j*(1+i/10), mtry = 30)
    # Predict
    pred = predict(forest3)
    # Calculate RMSE
    rmse_analysis = sqrt(mean((pred-trydata$price)^2))
...
colnames(rmse) = c("rmse","samplesize","mtry")
ggplot(aes(x = mtry, y = rmse, color = as.factor(samplesize)), data = rmse) +
  geom_point()
