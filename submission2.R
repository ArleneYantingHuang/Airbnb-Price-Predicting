...
for (i in 1:69) {
  if (length(levels(data[,i])) > 50) {
    print(i)
    print(length(levels(data[,i])))
  }
}
names = names(data)
names[c(1,3,5,10,11,12,14,16,18,41,49,50)]
#randomforest2
library(stringr)
...
names = data.frame()
for (i in 1:60) {
  names[i,1] = colnames(data)[i]
  names[i,2] = class(data[,i])
}
...
set.seed(1213)
forest2 = randomForest(price~., data = data, ntree = 1000)
# Predict
...
scoringData = rbind(data[,-59], scoringData)
scoringData = scoringData[36383:45592,]
